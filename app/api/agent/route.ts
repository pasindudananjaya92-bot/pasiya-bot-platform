...
export async function POST(req: Request) {
  ...
  if (!apiKey) {
    return new Response(
      JSON.stringify({ error: "GROK_API_KEY / XAI_API_KEY missing on Vercel" }),
      ...
      try {
        for (let round = 0; round < maxRounds; round++) {
          send({
            type: "log",
            level: "info",
            text: `Grok round ${round + 1}/${maxRounds}`,
          });
          ...
          if (!res.ok) {
            ...
            send({
              type: "error",
              text: `xAI ${res.status}: ${errText.slice(0, 400)}`,
            });
            break;
          }
        ...
        if (memoryId) {
          ...
          send({ type: "log", level: "mem", text: `Memory saved: ${memoryId}` });
        }
